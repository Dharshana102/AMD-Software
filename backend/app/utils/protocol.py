class UARTProtocol:
    ...
    CMD_GET_RPI_IP = "GET_RPI_IP"           # Get RPi IP address
    CMD_GET_RPI_MAC = "GET_RPI_MAC"         # Get RPi MAC address
    CMD_GET_RPI_INFO = "GET_RPI_INFO"       # Get both IP and MAC
    CMD_RPI_STATUS = "RPI_STATUS"           # Get RPi connection status
    ...
    CMD_HOST_CONTROL = "HST"                # HST:DEVICE:ACTION;
    CMD_HOST_STATUS = "HST_STATUS"          # HST_STATUS:DEVICE;
    CMD_HOST_STATUS_ALL = "HST_STATUS_ALL"  # HST_STATUS_ALL;
    ...
    def parse_response(response: str) -> dict:
        ...
        - "IP:192.168.1.100;" → {"success": True, "type": "rpi_ip", "ip_address": "192.168.1.100"}
        - "MAC:AA:BB:CC:DD:EE:FF;" → {"success": True, "type": "rpi_mac", "mac_address": "AA:BB:CC:DD:EE:FF"}
        """
        if not response:
        ...
        # ============ NEW: RPI RESPONSE HANDLERS ============
 
        # Handle RPi IP response: "IP:192.168.1.100;"
        if response.startswith("IP:"):
            ip_address = response.split(":", 1)[1].strip()
            return {
                "success": True,
                "type": "rpi_ip",
        ...
        # Handle RPi MAC response: "MAC:AA:BB:CC:DD:EE:FF;"
        if response.startswith("MAC:"):
            mac_address = response.split(":", 1)[1].strip()
            return {
                "success": True,
                "type": "rpi_mac",
        ...
        # Handle RPi Info response: "IP:192.168.1.100, MAC:AA:BB:CC:DD:EE:FF;"
        if "IP:" in response and "MAC:" in response:
            result = {"success": True, "type": "rpi_info"}
            parts = response.split(",")
            for part in parts:
                part = part.strip()
                if part.startswith("IP:"):
                    result["ip_address"] = part.split(":", 1)[1].strip()
                elif part.startswith("MAC:"):
                    result["mac_address"] = part.split(":", 1)[1].strip()
            return result
 
        # Handle RPi status response: "RPI_STATUS:ONLINE;" or "RPI_STATUS:OFFLINE;"
        if response.startswith("RPI_STATUS:"):
            status = response.split(":", 1)[1].strip().lower()
            return {
                "success": True,
                "type": "rpi_status",
                "status": status
        ...
        # Parse plug status: "DP_1:PLUGED;" or "DP_1:UNPLUGED;"
        ...
        # Parse host status response: "HOST_STATUS:ONLINE;"
        if response.startswith("HOST_STATUS:"):
            status = response.split(":", 1)[1].strip().lower()
            return {
                "success": True,
                "type": "host_status",
        ...
        # Parse all hosts status: "ALL_HOSTS:ONLINE,ONLINE,OFFLINE,ONLINE;"
        if response.startswith("ALL_HOSTS:"):
            statuses = response.split(":", 1)[1].split(",")
            return {
                "success": True,
                "type": "all_hosts_status",
                "devices": [s.strip().lower() for s in statuses]
            }
